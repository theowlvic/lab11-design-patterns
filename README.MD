...
to the _operate method in EngineBase Class in engine_base.py (line 54)
to the __operate_on_text method in EngineBase Class in engine_base.py (line 86)
to the create_operator_class in operators_factory.py (line 133)
```
...
The create_operator_class uses a strategy design pattern to choose which operator to use depending on the operator name. It uses a Dictionary class and to map the operator to the right name based on the operator type. This way there is no need for any if/elif branches.
